...
# pre_stage_dirs=("$TOP_DIR/plugins/"*"/pre-stage/")
# post_stage_dirs=("$TOP_DIR/plugins/"*"/post-stage/")

# shellcheck disable=SC2034,SC2207
IFS=$'\n' sorted=($(sort <<<"${plugin_dirs[*]}")); unset IFS
# # shellcheck disable=SC2034,SC2207
...
#
# 1. Scan following content from all subdirectories under plugins
...
run_pre_stage() {
    for path_item in "${plugin_dirs[@]}"
    do
        if [[ -d "$path_item/pre-stage" && -f "$path_item/pre-stage/host_run.sh" ]]; then
            info "Execute the host_run.sh at $path_item/pre_stage"
            chmod +x $path_item/pre_stage/host_run.sh
            $path_item/pre_stage/host_run.sh
        fi
    done
}
...
run_post_stage() {
    for path_item in "${plugin_dirs[@]}"
    do
        if [[ -d "$path_item/post-stage" && -f "$path_item/post-stage/host_run.sh" ]]; then
            info "Execute the host_run.sh at $path_item/post_stage"
            chmod +x $path_item/post_stage/host_run.sh
            $path_item/post_stage/host_run.sh
        fi
    done
...
clean_plugins() {
    for path_item in "${plugin_dirs[@]}"
    do
        if [[ -d "$path_item/post-stage" && -f "$path_item/post-stage/clean_up.sh" ]]; then
            info "Execute the clean_up.sh at $path_item/pre_stage/"
            chmod +x $path_item/pre_stage/clean_up.sh
            $path_item/pre_stage/clean_up.sh
        fi
    done
}
